...
class Tokenizer:
    ...
    # Tokenize the input
        ...
        while True:
            try:
                while re.search('\s', self.get_next_character()):
                    self.increment_pointer()
                ...
                if not ch:
                    ...
                    tokens.append(("$", "$" ,self.start_line_num+1))
                    return tokens

                token, token_name = self.get_next_token()
            
                tokens.append((token_name, token, self.start_line_num))
                
            except LexicalError as e:
                print(f'Lexical error: {e} - Line: {self.start_line_num} - Column: {self.start_col_num}')
                self.token_buffer = ''
